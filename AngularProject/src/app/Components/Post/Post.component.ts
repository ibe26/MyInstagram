...
export class PostComponent implements OnInit {
  ...
  ngOnInit() {
    this._isLiked = this.didUserLikePost();
  }
  ...
  public get IsCommentHovered(): boolean {
    return this._commentHovered;

  }

  public SwitchIsLiked() {
    if (isNaN(this.Post.likeCount)) {
      this.postService.likePost({ userID: this._userTest.userID, postID: this.Post.id });
      this.Post.likeCount = 1;
      this._isLiked = true;
      return;
    }
    this._isLiked ? this.postService.unlikePost({ userID: this._userTest.userID, postID: this.Post.id }) :this.postService.likePost({ userID: this._userTest.userID, postID: this.Post.id })
